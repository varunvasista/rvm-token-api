  ...
  await client.connect();
    ...
    await client.request({
      ...
      if (xrpBalance > 10) {
        ...
        res.status(200).json({ "status": "success", "reserve":totalreserve,"balance": finalbalance });
        
      } else if(xrpBalance <= 10){
          res.status(200).json({ "status": "success", "reserve":Math.round(xrpBalance), "balance": 0 });
      }else{
          res.status(200).json({ "status": "success", "reserve":0, "balance": xrpBalance });
      }
    })
    .catch((error)=>{
...
await client.connect();
  ...
  // console.log("Signed blob:", signed.tx_blob)
    ...
    .catch((error)=>{
      if (error.data.error == 'actNotFound') {
        const xrpBalance = 0;
        res.status(200).json({ "status": "success", "balance": xrpBalance });
      } else if(error.data.error == 'actMalformed'){
        res.status(201).json({ "status": "error", "message": "Invalid XRP Account"});
      }else{
        res.status(201).json({ "status": "error", "message": "Something went wrong! try again"});
